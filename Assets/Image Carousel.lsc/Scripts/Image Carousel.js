// Helper to append debug text to the on-screen text component
//function appendDebugText(msg) {
    // Replace all numbers in the string with their two-decimal representation
//    msg = msg.replace(/(-?\d*\.?\d+)/g, function(match) {
...
//    });
//    if (script.debugText) {
//        var current = script.debugText.text || "";
//        script.debugText.text = current + "\n" + msg;
//    }
//}
// ImageCarousel.js
// Version 16.0.0
...
function initInteraction(root, st) {
    ...
    carouselUI.createEvent("TouchStartEvent").bind(function (args) {
        if (autoMode || touching || getTime() < touchCooldownUntil) {
            return;
        }
    ...
    carouselUI.createEvent("TouchMoveEvent").bind(function (args) {
        ...
        var screenPos = args.getTouchPosition();
        const pos = st.screenPointToLocalPoint(screenPos);        
        if (lastTouch.lastMoveX !== null && (Math.abs(pos.x - lastTouch.lastMoveX) > 1) || (lastTouch.lastMoveX<-0.4 && pos.x>0.4)||(lastTouch.lastMoveX>0.4 && pos.x<-0.4)) {
           // appendDebugText("Glitch detected - resetting carousel");
            resetCarousel();
            return;
        }
        //appendDebugText("continue on move" + lastTouch.lastMoveX + " " + pos.x);
        lastTouch.lastMoveX = pos.x;
    ...
    carouselUI.createEvent("TouchEndEvent").bind(function (args) {
        if (!touching) {
            lastTouch.lastMoveX = null;
            return;
        }
        touching = false;
        lastTouch.lastMoveX = null; // Reset for next touch
        if (items.length < 1) {
            return;
        }
...
// Resets carousel by destroying and reinitializing everything
function resetCarousel() {
    // Destroy the old carousel SceneObject if it exists
    if (carouselRoot) {
        carouselRoot.destroy();
        carouselRoot = null;
    }
    ...
    if (script.debugText) script.debugText.text = "";
    // Re-initialize everything by calling init()
    if (init()) {
        // Call start to position items immediately
        start();
    }
}
